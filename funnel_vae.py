...
class Funnel_VAE(PreTrainedModel):
    base_model_prefix = "t5_vae"

    def __init__(self, config, funnel_model, vae, set_seq_size, tokenizer):
    ...
    def _decoder_logits(self, decoder_input_ids, encoding):
        sequence_output = self.funnel_model.decoder(input_ids=decoder_input_ids, encoder_hidden_states=encoding)[0]
        # Rescale output before projecting on vocab
        # See https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/transformer/transformer.py#L586
        sequence_output = sequence_output * (self.funnel_model.model_dim ** -0.5)
        logits = self.funnel_model.lm_head(sequence_output)
        return logits

    def decoder_loss(self, labels, encoding, ignore_index=-100):
        decoder_input_ids = self.funnel_model._shift_right(labels)
    ...
    def get_latent(self, input_ids):
        ...
        encoding = self.funnel_model.encoder(input_ids=input_ids, attention_mask=attention_mask)[0]
...
def _get_t5_vae_requirements(model_args, training_args):
    funnel_model, tokenizer = _get_t5_model(model_args.set_seq_size, model_args.vocab_file)
    vae = _get_ae(funnel_model.config, model_args, training_args)
    return funnel_model, tokenizer, vae


def new_t5_vae(model_args, training_args):
    funnel_model, tokenizer, vae = _get_t5_vae_requirements(model_args, training_args)
    return Funnel_VAE(funnel_model.config, funnel_model, vae, model_args.set_seq_size, tokenizer)


def load_t5_vae(model_args, training_args):
    funnel_model, tokenizer, vae = _get_t5_vae_requirements(model_args, training_args)
    return Funnel_VAE.from_pretrained(
        model_args.model_path,
        config=funnel_model.config,
